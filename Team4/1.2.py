...
f=open("/var/www/html/number.html","w")
...
def main():
    ...
    while (True):
        while (True):
            video_device1 = cv2.VideoCapture(1)
            #actual_frame_width = video_device1.get(cv2.CAP_PROP_FRAME_WIDTH)
            #actual_frame_height = video_device1.get(cv2.CAP_PROP_FRAME_HEIGHT)
            ...
            if ((video_device1 == None) or (not video_device1.isOpened())):
                print ('Could not open video1 device.  Make sure file exists:')
            ...
            #video_device2 = cv2.VideoCapture(2)
            #actual_frame_width = video_device2.get(cv2.CAP_PROP_FRAME_WIDTH)
            #actual_frame_height = video_device2.get(cv2.CAP_PROP_FRAME_HEIGHT)
            #print ('actual video resolution: ' + str(actual_frame_width) + ' x ' + str(actual_frame_height))
            ...
            #frame_count = 0
            #start_time = time.time()
            #video_device3 = cv2.VideoCapture(3)

            #actual_frame_width = video_device3.get(cv2.CAP_PROP_FRAME_WIDTH)
            #actual_frame_height = video_device3.get(cv2.CAP_PROP_FRAME_HEIGHT)
            #print ('actual video resolution: ' + str(actual_frame_width) + ' x ' + str(actual_frame_height))
            ...
            while True :
                ...
                input_img = np.ones((450,450，3),dtype=np.uint8)
                ret_val1, input_image1 = video_device1.read()
                input_image1 = cv2.resize(input_image1, (450, 150), cv2.INTER_LINEAR)
                input_img[0:150,0:450]=input_image1[0:150,0:450]
                if (not ret_val1):
                    end_time = time.time()
                    print("No image from from video device1, exiting")
                    break
                #ret_val2, input_image2 = video_device2.read()
                #input_image2 = cv2.resize(input_image2, (450, 150), cv2.INTER_LINEAR)
                #input_img[150:300,0:450]=input_image2[0:150,0:450]
                #if (not ret_val2):
                    #end_time = time.time()
                    #print("No image from from video device2, exiting")
                    #break
                #ret_val3, input_image3 = video_device3.read()
                #input_image3 = cv2.resize(input_image3, (450,150), cv2.INTER_LINEAR)
                #input_img[300:450,0:450]=input_image3[0:150,0:450]
                ...
                # and overlay them directly on the video image returned from video device.
                 display_image = cv2.resize(display_image, (450, 450),
                                           cv2.INTER_LINEAR)
                # update the GUI window with new image
                cv2.imshow(cv_window_name, input_img)
